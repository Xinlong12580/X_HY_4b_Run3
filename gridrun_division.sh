...
# MAIN FUNCTION
echo python run_division.py $* | tee -a $root_dir/out.txt
python run_division.py $* | tee -a $root_dir/out.txt
ls | tee -a $root_dir/out.txt
# move all snapshots to the EOS (there will only be one)
xrdcp -f *.root root://cmseos.fnal.gov//store/user/xinlong/XHY4bRun3_2022_division/
xrdcp -f $root_dir/out.txt root://cmseos.fnal.gov//store/user/xinlong/XHY4bRun3Run3_2022_division/
