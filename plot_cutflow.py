...
with open("pkls/hists_optmize_wp_cutflow.pkl", "rb") as f:
    cutflows = pickle.load(f)
cuts = [cut for cut in cutflows]
print(cuts)
#------------------------------ making plots ------------------------------------------------------------
...
for year in years:
    ...
    width = 0.3
    for process in cutflows[cuts[0]][year]:
    ...
    ax.set_yscale("log")
    ax.legend()
    fig.savefig(f"plots/plots_wp_cutflow/{year}_cutflow.png")
...
for year in years:
    ...
    ax.set_xticklabels(cuts, rotation=45, ha='right')
    ax.legend()
    fig.savefig(f"plots/plots_wp_cutflow/{year}_SoverSqrtB.png")
