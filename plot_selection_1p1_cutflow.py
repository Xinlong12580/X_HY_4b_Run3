...
#--------------------------------defining parameters---------------------------------------------------------
cuts = ["BeforeSkim", "Skim", "GoldenJson", "SkimOf1p1", "LeptonVeto", "TriggerCut", "FlagCut", "FatJetID", "FatJetPt", "FatJetMass", "DeltaEta", "MassJJ", "HiggsMatch"]
cutflows = {}
...
for year in years:
    ...
    ax.set_yscale("log")
    ax.legend()
    fig.savefig(f"plots_selection_cutflow/{year}_cutflow.png")
...
for year in years:
    ...
    ax.set_xticklabels(cuts, rotation=45, ha='right')
    ax.legend()
    fig.savefig(f"plots_selection_cutflow/{year}_SoverSqrtB.png")
