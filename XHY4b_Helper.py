...
def load_TH1(data_files, template_files, years, bins, processes, MC_weight, save_name, root_save_name, Xsec_json, signal_json, hist_name = "Default"):
    h_data = {}
    for year in years:
        h_data[year] = {}
        for column in bins:
            h_data[year][column] = ROOT.TH1D(f"{hist_name}_{year}_Data_Data_{column}_1", f"{hist_name}_{year}_Data_Data_{column}_1", len(bins[column]) - 1, bins[column])
                ...
                if subprocess == "*":
                    if "SignalMC_" in process:
                        for _subprocess in signal_json[year][process]:
                            ...
                            for column in bins:
                                h_BKGs[year][process][_subprocess][column] = ROOT.TH1D(f"{hist_name}_{year}_{process}_{_subprocess}_{column}_{MC_weight}", f"{hist_name}_{year}_{process}_{_subprocess}_{column}_{MC_weight}", len(bins[column]) - 1, bins[column])
                        break
                    elif "MC_" in process:
                        for _subprocess in Xsec_json[process]:
                            h_BKGs[year][process][_subprocess] = {}
                    ...
                    for column in bins:
                        h_BKGs[year][process][subprocess][column] = ROOT.TH1D(f"{hist_name}_{year}_{process}_{subprocess}_{column}_{MC_weight}", f"{hist_name}_{year}_{process}_{subprocess}_{column}_{MC_weight}", len(bins[column]) - 1, bins[column])
...
def load_TH2(data_files, template_files, years, bins, processes, MC_weight, Xsec_json, signal_json, hist_name = "Default"):
    h_data = {}
    for year in years:
        h_data[year] = {}
        for column in bins:
            h_data[year][column] = ROOT.TH2D(f"{hist_name}_{year}_Data_Data_{column}_1", f"{hist_name}_{year}_Data_Data_{column}_1", len(bins[column]["x"]) - 1, bins[column]["x"], len(bins[column]["y"]) - 1, bins[column]["y"])
                ...
                if subprocess == "*":
                    if "SignalMC_" in process:
                        for _subprocess in signal_json[year][process]:
                            h_BKGs[year][process][_subprocess] = {}
                            for column in bins:
                                h_BKGs[year][process][_subprocess][column] = ROOT.TH2D(f"{hist_name}_{year}_{process}_{_subprocess}_{column}_{MC_weight}", f"{hist_name}_{year}_{process}_{_subprocess}_{column}_{MC_weight}", len(bins[column]) - 1, len(bins[column]["x"]) - 1, bins[column]["x"], len(bins[column]["y"]) - 1, bins[column]["y"])
                        break
                    elif "MC_" in process:
                        for _subprocess in Xsec_json[process]:
                            h_BKGs[year][process][_subprocess] = {}
                            for column in bins:
                                h_BKGs[year][process][_subprocess][column] = ROOT.TH2D(f"{hist_name}_{year}_{process}_{_subprocess}_{column}_{MC_weight}", f"{hist_name}_{year}_{process}_{_subprocess}_{column}_{MC_weight}", len(bins[column]["x"]) - 1, bins[column]["x"], len(bins[column]["y"]) - 1, bins[column]["y"])
                        break
                else:
                    h_BKGs[year][process][subprocess] = {}
                    for column in bins:
                        h_BKGs[year][process][subprocess][column] = ROOT.TH2D(f"{hist_name}_{year}_{process}_{subprocess}_{column}_{MC_weight}", f"{hist_name}_{year}_{process}_{subprocess}_{column}_{MC_weight}", len(bins[column]["x"]) - 1, bins[column]["x"], len(bins[column]["y"]) - 1, bins[column]["y"])
