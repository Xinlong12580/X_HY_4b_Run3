...
data_files = os.listdir("optimize_wp")
data_files = ["optimize_wp/" + f for f in data_files]
...
rdf = ROOT.RDataFrame("Cutflow", data_files[0])
columns = rdf.GetColumnNames()
cuts = [str(column) for column in columns if "SRCut" in str(column)]
cutflows = {}
...
save_name = "pkls/hists_optmize_wp_cutflow.pkl"
...
for data_file in data_files:
    for year in years:
        ...
        if (year + "_" ) in data_file:
            for process in processes:
                if process in data_file:
                    for subprocess in cutflows[cuts[0]][year][process]:
                        if subprocess in data_file:
                            print(data_file)
                            if "SignalMC_" in process:
                                Xsec = 1
