...
for file in raw_nano/files/*.txt; do
    ...
    if [[ $pass == 1 ]]; then
        if [[ "$file" == *"2022__"* ]]; then
            ./gen_args_skim.sh $file 2022 $output $n_files
        elif [[ "$file" == *"2022EE__"* ]]; then
            ./gen_args_skim.sh $file 2022EE $output $n_files
        elif [[ "$file" == *"2023__"* ]]; then
            ./gen_args_skim.sh $file 2023 $output $n_files
        elif [[ "$file" == *"2023BPix__"* ]]; then
            ./gen_args_skim.sh $file 2023BPix $output $n_files
        fi
    fi
done
